...
stowGoody [] _ bp = 
    bp
stowGoody (Nothing : _) 0 bp = 
    bp
stowGoody (Just g : _) 0 (Backpack m) =
    Backpack (update g (1 + applyElse m g 0) m)
stowGoody (_ : gs) n bp =
    stowGoody gs (n-1) bp
...
transition poke (State path posn dir bp) = 
    let newDir = if poke then not dir else dir 
        move = if newDir then 1 else -1
        newPosn = max 0 (min (posn + move) (length path - 1))
    in
    State (deleteGoody path newPosn) newPosn newDir (stowGoody path newPosn bp)
