...
apply (Add a b ps) name = 
    if a == name then Just b
    else apply ps name
...
applyElse (Add a b ps) name d =
        if a == name then b
        else applyElse ps name d

-- (update x y m) is a map m' such that
-- apply m' x ==> y
-- apply m' z ==> apply m z   if z is different than x
update :: Eq a => a -> b -> Map a b -> Map a b 
update old new Empty = (Add old new Empty)
update old new (Add a b ps) = 
    if old == a then (Add a new ps)
    else Add a b (update old new ps)
...
deleteGoody (x : xs) num = 
    if num == 0 then (Nothing : xs)
    else (x : deleteGoody xs (num - 1))
...
stowGoody [] _ bp = bp
stowGoody (Nothing : xs) _ bp = bp
stowGoody (Just x : xs) 0 (Backpack m) =
    Backpack (update x (1 + applyElse m x 0) m)
stowGoody (_ : xs) num bp =
    stowGoody xs (num - 1) bp
...
transition True (State pathList pos goingRight bp) = 
    let newBp = stowGoody pathList pos bp
        pathChange = updatePath pathList pos
        newDirection = not goingRight
    in
        if newDirection then
            if 1 + pos < length pathList then 
                (State pathChange (pos + 1) newDirection newBp)
            else (State pathChange pos newDirection newBp)
        else 
            if pos - 1 >= 0 then 
                (State pathChange (pos - 1) newDirection newBp)
            else (State pathChange pos newDirection newBp)
transition False (State pathList pos goingRight bp) =
    let newBp = stowGoody pathList pos bp
        pathChange = updatePath pathList pos
    in
        if goingRight then
            if 1 + pos < length pathList then 
                (State pathChange (pos + 1) goingRight newBp)
            else (State pathChange pos goingRight newBp)
        else 
            if pos - 1 >= 0 then 
                (State pathChange (pos - 1) goingRight newBp)
            else (State pathChange pos goingRight newBp)

updatePath :: [Maybe Goody] -> Int -> [Maybe Goody]
updatePath (x : xs) 0 = (Nothing : xs)
updatePath (x : xs) pos =
    (x : updatePath xs (pos - 1))
