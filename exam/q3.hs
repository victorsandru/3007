...
sumP :: [Point] -> Point
sumP ps = 
    foldr (\acc p -> addP acc p) (Point 0 0) ps
...
nearest (z : ps) = 
    foldr 
        (\acc p -> if p |<=| acc then p else acc)
        (Point 10000 10000)
        ((z : ps) :: [Point])
...
zipOp ::  (a -> a -> a) -> [a] -> [a] -> [a]
zipOp op [x] (y : ys) = [op x y]
zipOp op (x : xs) [y] = [op x y]
zipOp op (x : xs) (y : ys) = op x y : zipOp op xs ys
...
instance BinOp a => BinOp (Array a) where
    zero = Array []
    Array xs <.> Array ys = 
        Array  $ zipOpWithPadding zero (<.>) xs ys
