...
profCourseAvgs ratings profName = 
    map (\c -> (c, profCourseAvg ratings profName c)) (profCourses ratings profName)
...
profAvg ratings profName = 
    average (map (\a -> snd a) (profCourseAvgs ratings profName))

-- A mapping giving the average score for each prof
allProfAvgs :: [Rating] -> [(Name, Int)]
allProfAvgs ratings = 
    map (\p -> (p, profAvg ratings p)) (profs ratings)

-- The average score given by a student.
studentAvg :: [Rating] -> Name -> Int
studentAvg ratings studentName = 
    average (map (\s -> ratingScore s) (studentRatings ratings studentName))

-- The average of all student averages.
avgStudentAvg :: [Rating] -> Int    
avgStudentAvg ratings = 
    average (map (\a -> studentAvg ratings a) (map (\r -> ratingStudent r) ratings))
